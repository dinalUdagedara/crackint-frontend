...
type MessageBubbleProps = {
  message: Message
  isPending?: boolean
  /** Logged-in user’s profile for USER message avatars */
...
export const MessageBubble: React.FC<MessageBubbleProps> = ({
  ...
  const bubbleClassName = `text-[15px] ${isUser
  ...
  return (
    ...
    >
      ...
      {isUser ? (
        <div className="flex max-w-[90%] min-w-0 items-start gap-2 sm:gap-3">
          <div className={`min-w-0 max-w-full flex-1 ${bubbleClassName}`}>
            {bubbleContent}
          </div>
          <div className="shrink-0 mt-0.5">
            <UserAvatar
              ...
              email={userEmail}
              size="sm"
              className="ring-border/80"
            />
          </div>
        </div>
      ) : (
        <div className={`max-w-[85%] ${bubbleClassName}`}>{bubbleContent}</div>
      )}
    </div>
  )
}
